...
The code also must be compiled with the newest version of gcc or a compiler that implements both C++ threading and openMP with POSIX threads. This is becuase that we have put some variables in thread_local storage where the openMP threadprivate clause wouldn't allow us to do so.
