...
vector<int> avi;                                        //index of all neurons
...
void syncinit(){
    #pragma omp parallel num_threads(Lthreadz) proc_bind(spread)
    {
        ...
        for(int y = 0;y < NNs - 1;y++){
            ...
            vector<int> vec = avi;
            vec.erase(vec.begin() + y,vec.end());
            for(int x = 0; x < W1i[y].size();x++){
            ...
            for(int i = 0; i < connectn; i++){
                ...
                it = true;
                for(int z = 0; z < W1i[y].size();z++){
                    if(vec[rrn] > W1i[y][z]){
                        W1i[y].insert(W1i[y].begin() + z ,vec[rrn]);
                        in = al(twisting); 
                        W1s[y].insert(W1s[y].begin() + z ,in);
                        vec.erase(vec.begin()+rrn);
                        it = false;
                        break;
                    }
                }
                if(it){
                    W1i[y].emplace_back(vec[rrn]);
                    in = al(twisting); 
        ...
        for(int y = 1;y < NNs ;y++){
            ...
            vector<int> vec = avi;
            for(int x = 0; x < W2i[y].size();x++){
                vec.erase(vec.begin() + W2i[y][x]);
            }
            ...
            for(int i = 0; i < connectn; i++){
                rrn = twisting() % vec.size();
                it = true;
                for(int z = 0; z < W2i[y].size();z++){
                    if(vec[rrn]>W2i[y][z]){
                        W2i[y].insert(W2i[y].begin() + z , vec[rrn]);
                        in = al(twisting); 
                        W2s[y].insert(W2s[y].begin() + z , in);
                        vec.erase(vec.begin()+rrn);
                        it = false;
                        break;
                    }
...
void savebinf(){
    ofstream avibin("avi.bin",ofstream::trunc); 
    boost::archive::binary_oarchive  iavbin(avibin); 
    iavbin << avi; 
    avi.clear();
...
void savexmlf(){ 
    ofstream avixml("avi.xml",ofstream::trunc);  
    boost::archive::xml_oarchive  iavxml(avixml);  
    iavxml << BOOST_SERIALIZATION_NVP(avi);  
    avi.clear();
